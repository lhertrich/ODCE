...
class Detr(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if freeze_backbone:
            for param in self.detr_model.backbone.parameters():
                param.requires_grad = False
            print("DETR backbone frozen.")
    ...
    def forward(
        ...
        decoder_outputs = self.detr_model(input)
    ...
    def predict_with_encodings(
        ...
        target_sizes = torch.tensor(
            (input.shape[2], input.shape[3]), device=raw_logits.device
        )  # Shape [batch_size, 2]
