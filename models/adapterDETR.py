...
class AdapterDetr(nn.Module):
    ...
    def forward(
        ...
        batch_size, transformed_seq_len, hidden_dim = (
            transformed_encoder_hidden_states.shape
        )

        query_pos_embed = self.query_pos_embed_weight.unsqueeze(0).repeat(
            batch_size, 1, 1
        )
        ...
        if encoder_pos_embed is None:
            encoder_pos_embed = get_sinusoidal_positional_embedding(
                batch_size,
                transformed_seq_len,
                hidden_dim,
                transformed_encoder_hidden_states.device,
            )
        ...
        raw_logits = self.classification_head(
            last_hidden_state
        )  # Shape: (batch_size, num_queries, num_classes)
        raw_pred_boxes = self.bbox_regression_head(
            last_hidden_state
    ...
    def predict_with_encodings(
        ...
        target_sizes = torch.tensor(
            original_image_size, device=raw_logits.device
        )  # Shape [batch_size, 2]
        ...
        processed_results = self.processor.post_process_object_detection(
            outputs=output_object, target_sizes=target_sizes, threshold=threshold
        )
        ...
        for i, detections_dict in enumerate(processed_results):
            ...
            for score, label_id, box in zip(
                ...
                img_detections.append(
                    {"score": round(score.item(), 3), "label": label, "box": box_list}
                )
