...
class RefL4Dataset(Dataset):
    def __init__(
        ...
        if load_features and self.feature_dir:
            ...
            assert self.vision_features is not None and self.text_features is not None, \
    ...
    def _load_dataset(self):
        ...
        if self.load_images:
            self.images = self._load_images_from_tar(f"{self.dataset_path}/{self.images_file}")
        all_splits = concatenate_datasets([self.dataset["val"], self.dataset["test"]])
        self.dataset["all"] = all_splits
        labels = set(self.dataset['all'].unique("ori_category_id"))
        # 3. Create sorted class list and mapping
                    ...
                    if member.isfile() and member.name.lower().endswith(("jpg", "jpeg", "png", "webp")):
                        ...
                        if f:
                            try:
                                ...
                                logging.warning(f"Failed to load {member.name}: {str(e)}")
        except Exception as e:
    ...
    def _load_clip_features(self):
        try:
            ...
            assert len(self.vision_features) == len(self.dataset[self.split]), \
    ...
    def __getitem__(self, idx):
        ...
        data["labels"] = torch.tensor(self.label2id[data["ori_category_id"]])
...
# Usage example
def get_dataloader(dataset_name, feature_path, split="val", batch_size=32, shuffle=False, device='cuda'):
    """
...
if __name__ == "__main__":
    ...
    dataloader = get_dataloader(dataset_name='JierunChen/Ref-L4', feature_path=feature_path)
